class BurndownsController < RedmineScrumController
  ...
  def index
    @burndown = BurndownFlot.area('Burndown') do |f|
      ...
      f.series_for("Reopens", @sprint.burndown, :x => :sprint_day, :y => :display_reopens, :tooltip => lambda {|r| "#{r.display_reopens} stories reopened" }, :options => {:points => {:show => true}, :lines => {:show => false}})
    end
  end
end