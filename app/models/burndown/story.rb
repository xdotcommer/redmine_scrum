class Burndown::Story < Burndown
  ...
  #   return true if sprint == Sprint.backlog || user.nil?
  #   
  ...
  #   end
  #   
  #   snapshot.committed_count       = Commitment.count(:conditions => ["user_id=? AND sprint_id=?", user.id, sprint.id])
  #   snapshot.committed_point_count = Commitment.sum(:story_points, :conditions => ["user_id=? AND sprint_id=?", user.id, sprint.id])
  # 
  #   snapshot.save
  # end
end