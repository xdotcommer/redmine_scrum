  ...
  module IssuePatch
    def self.included(base) # :nodoc:
      ...
      base.class_eval do
        ...
        named_scope   :work_in_progress, :conditions => ['custom_values.value = ? OR custom_values.value IS NULL AND custom_fields.name = ?', "Work in Progress", 'Story Readiness'], :include => {:custom_values =>  :custom_field}
    ...
    module InstanceMethods
      ...
      def ready_for_review?
        ! custom_values.find(:first, :conditions => ['custom_values.value = ? AND custom_fields.name = ?', "Ready for Review", 'Story Readiness'], :include => :custom_field).empty?
      end
